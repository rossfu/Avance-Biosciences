...
def update_db(source, dest, archive_dest, orig_hash, ctime, owner, mtime, instrument, asset, proj, summary):
    # Update DB
    db_insert = "INSERT INTO t5_nifi_datasafe (source, dest, archive_dest, hash, ctime, owner, mtime, instrument, asset, proj, summary) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    db_insert_vals = (source, dest, archive_dest, orig_hash, ctime, owner, mtime, instrument, asset, proj, summary)
...
if work_folder_backup_status == True and archive_backup_status == True:
    update_db(og_filepath, work_folder_dest_path, archive_dest_path, orig_hash, ctime, username, mtime, instrument, asset, idproj, 'new')

elif modified_status == True:
    update_db(og_filepath, work_folder_dest_path, '', orig_hash, ctime, username, mtime, instrument, asset, idproj, 'modified')
