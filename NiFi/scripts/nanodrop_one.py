...
# in:  /mnt/cifs/nanodrop8000/
# out: /mnt/cifs/dataarchive/2023/NanoDrop8000/
...
try:
	...
	print("nanodrop_one.py file-path file-name")
	sys.exit()
...
filepath_45 = "\\\\45drives\\hometree\\DataArchive\\2023\\NanoDrop1000\\"
filepath_45 += filename
print(filepath_45)
...
with open(og_filepath, 'r', encoding='ISO-8859-1') as f:
	...
	for row in tsv_reader:
		if "Nucleic Acid" in _module:
			...
			a260 = good_cols[6]
			a280 = good_cols[7]
			a260_280 = good_cols[8]
			a260_230 = good_cols[9]
			#a230 = good_cols[9]
			#serial = good_cols[14] 

			try:
				# Insert into DB:
				db_insert = f"INSERT INTO t6_nanodrop (`NanodropType`,`Serial`, `ProjectID`, `SampleID`, `UserID`,  `Date`, `Time`, `Conc`, `Units`, `A260`, `A280`, `260_280`, `260_230`, `230`, `NA_Type`, `module`, `path`, `software`,`filepath`) VALUES ('8000', '{serial}', '{_project_id}', '{sampleid}','{userid}','{date_str}','{time_str}',{conc},'{_units}',{a260},{a280},{a260_280},{a260_230},{a230},'{_na_type}','{_module}','{_path}','{_software}','{filepath_45}')"
				print(db_insert)
			...
			try:
				# Insert into DB:
				db_insert = f"INSERT INTO t6_nanodrop (`Serial`, `ProjectID`, `SampleID`, `UserID`,  `Date`, `Time`, `cell_baseline`, `cell_600`, `280`,`module`, `path`, `software`, `filepath`) VALUES ('{serial}', '{_project_id}', '{sampleid}','{userid}','{date_str}','{time_str}',{_baseline}, {_cell_600}, {_cell_280}, '{_module}','{_path}','{_software}', '{filepath_45}')"
				print(db_insert)
				lims_db_cursor.execute(db_insert)
				lims_db.commit()
			except Exception as e:
