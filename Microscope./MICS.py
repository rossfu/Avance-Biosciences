...
# See if the project ID exists in avancelims.get_full_idproject view
# Need to make sure the view is also in prod
def checkProjectID(proj_id, cursor):
    query = 'SELECT `full_idprojects` FROM `get_full_idproject` WHERE `full_idprojects` = "{}"'.format(proj_id)
...
##        self.IP = 'limsnewprod.avancebio.com'
    ...
    def try_login(self, user, pw, proj_id, db, LIMS_Table, LIMS_IP):
        ...
        # Data input validation for correct project ID formatting
        match_check = re.search("[A-Z]{3}[0-9]{6}", proj_id)
        if not match_check or not len(proj_id) == 9:

            print('Incorrect Project ID Format. Project ID must have 3 capital letters followed by 6 numbers.\n')
            
            msg = QMessageBox()
            msg.setWindowTitle('LIMS')
            msg.setText('Incorrect Project ID Format. Project ID must have 3 capital letters followed by 6 numbers.')
            x = msg.exec()
...
##            lims_db.commit()
            ...
            # See if the project ID exists in avancelims.get_full_idproject view (only active after validation)
            if validated:
                ID_exists = checkProjectID(proj_id, lims_db_cursor)
                if not ID_exists:
                    print('Project ID could not be found in LIMS. Please try again with a different project ID.\n')
                
                    msg = QMessageBox()
                    msg.setWindowTitle('LIMS')
                    msg.setText('Project ID could not be found in LIMS. Please try again with a different project ID.')
                    x = msg.exec()
        ...
        if login_success:# Proceed
            ...
            global final_img_root_path
            final_img_root_path = self.config_settings[1]
...
##                    action.setData(i)
##                    menu.addAction(action)
##                action = menu.exec(self.mapToGlobal(btn_snap.pos())) #default 1
                self.hcam.Snap(0)
        ...
        try:
            ...
            if info.width > 0 and info.height > 0:
                ...
                try:
                    ...
                    now_time = str(now.replace(microsecond=0)).replace(':',';')


                    filepath = final_img_root_path + '\\' + projid + '_' + os.environ['USERNAME'] + '_' + now_time + '_capture{}.jpg'.format(self.count)
...
##                    import subprocess
##                    subprocess.call(['chmod', '-R', 'a-w', filepath])
##                    subprocess.check_call(["chmod -R a-w {path}".format(path=filepath)], shell=True)

                    
##                    # Change file permissions
##                    userx, domain, type = win32security.LookupAccountName ("", os.environ['USERNAME'])
##                    usery, domain, type = win32security.LookupAccountName ("", 'efu')
##
...
##                    dacl.AddAccessAllowedAce(win32security.ACL_REVISION, con.FILE_GENERIC_READ, usery)
##
##                    sd.SetSecurityDescriptorDacl(1, dacl, 0)   # may not be necessary
##                    win32security.SetFileSecurity(filepath, win32security.DACL_SECURITY_INFORMATION, sd)


##                    def restrict_delete_modify_permissions(file_path, trustee, deny_rights):
##                        try:
##                            # Get the file's existing DACL (Discretionary Access Control List)
##                            sd = win32security.GetFileSecurity(file_path, win32security.DACL_SECURITY_INFORMATION)
##
##                            # Get the SID for the specified trustee (user or group)
##                            trustee_sid = get_sid(trustee)
##
##                            if trustee_sid:
##                                # Create a new ACE (Access Control Entry) for the specified trustee
##                                ace = win32security.ACL()
##                                ace.AddAccessAllowedAce(win32security.ACL_REVISION, con.FILE_GENERIC_READ, trustee_sid)
##                                ace.AddAccessDeniedAce(win32security.ACL_REVISION, deny_rights, trustee_sid)
##
##                                # Add the new ACE to the existing DACL
##                                sd.SetSecurityDescriptorDacl(1, ace, 0)
##
##                                # Apply the modified DACL to the file
##                                win32security.SetFileSecurity(file_path, win32security.DACL_SECURITY_INFORMATION, sd)
##
##                                print(f"Permissions modified to restrict delete and modify for {trustee} on {file_path}")
##                        except Exception as e:
##                            print(f"Error modifying permissions: {e}")
##
##                    def get_sid(trustee):
##                        try:
##                            sid, _, _ = win32security.LookupAccountName(None, trustee)
##                            return sid
##                        except Exception as e:
##                            print(f"Error getting SID: {e}")
##                            return None
##
##                    # Example: Restrict delete and modify permissions for a specific user
##                    deny_rights = con.DELETE | con.FILE_GENERIC_WRITE  # Deny delete and modify rights
##
##                    restrict_delete_modify_permissions(filepath, os.environ['USERNAME'], deny_rights)



                    db_insert = "INSERT INTO avancelims.t5_microscope_cam_audit_trail (Path_to_IMG, Time_Stamp, User, Computer, Project_ID) VALUES (%s,%s,%s,%s,%s)"
                    db_insert_vals = (final_img_root_path + '\\' + projid + '_' + os.environ['USERNAME'] + '_' + now_time + '_capture{}.jpg'.format(self.count), datetime.now(), os.environ['USERNAME'], os.environ['COMPUTERNAME'], projid)
                    lims_db_cursor.execute(db_insert, db_insert_vals)
                    lims_db.commit()

                    
                    print('\nImg Saved to: ' + final_img_root_path + '\\' + projid + '_' + os.environ['USERNAME'] + '_' + now_time + '_capture{}.jpg'.format(self.count))          #\\45drives\hometree\Operations\Equipment\Microscope_Camera\ImageCaptures\<ProjectID>
