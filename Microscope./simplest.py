...
# the vast majority of callbacks come from swiftcam.dll/so/dylib internal threads
    @staticmethod
    def cameraCallback(nEvent, ctx):
        if nEvent == swiftcam.SWIFTCAM_EVENT_IMAGE:
    ...
    def CameraCallback(self, nEvent):
        if nEvent == swiftcam.SWIFTCAM_EVENT_IMAGE:
            ...
            except swiftcam.HRESULTException as ex:
                print('pull image failed, hr=0x{:x}'.format(ex.hr & 0xffffffff))
        else:
            print('event callback: {}'.format(nEvent))

    def run(self):
        a = swiftcam.Swiftcam.EnumV2()
        if len(a) > 0:
            ...
            self.hcam = swiftcam.Swiftcam.Open(a[0].id)
            if self.hcam:
                try:
                    width, height = self.hcam.get_Size()
                    bufsize = swiftcam.TDIBWIDTHBYTES(width * 24) * height
