...
wb_name = os.environ['USERNAME'] + '_AvroBio_DataAnalysis ' + str(now).replace(':',';') + ' .xlsx'
...
lil_format = workbook.add_format({'font_size':8, 'font_name':'Times New Roman'})
normal_format = workbook.add_format({'font_size':12, 'font_name':'Times New Roman'})
normal_text_wrapped_format = workbook.add_format({'font_size':12, 'font_name':'Times New Roman'})
normal_text_wrapped_format.set_text_wrap()
bold_format = workbook.add_format({'bold': True, 'font_size':12, 'font_name':'Times New Roman'})

Fancy_bold_format = workbook.add_format({'bold': True, 'font_size':14, 'font_name':'Times New Roman', 'align':'center'})
Fancy_bold_format.set_underline(1)

bold_border_format = workbook.add_format({'bold': True, 'border':1,'font_size':12, 'font_name':'Times New Roman', 'align':'left', 'valign':'vcenter'})
normal_border_format = workbook.add_format({'border':1, 'font_size':11, 'font_name':'Times New Roman', 'align':'left', 'valign':'vcenter'})
normal_border_format.set_text_wrap()




##header = '&L&12&"Times New Roman"SCS 5000&C&18&"Times New Roman"\nCERTIFICATE OF ANALYSIS\n\n\n\n&R&12&"Times New Roman"Effective Date: 1 Apr 2023'
...
lims_db_cursor.execute("SELECT * FROM coa_scripts_config WHERE Client = 'AvroBio'")
AvroBio_coa_config_settings = lims_db_cursor.fetchone()


sop = AvroBio_coa_config_settings[1]    # Only 4 Configs in v1?
assay_title = AvroBio_coa_config_settings[2]
pipeline = AvroBio_coa_config_settings[3]
vendor_version = AvroBio_coa_config_settings[4]
quality_statement = AvroBio_coa_config_settings[5]
testing_facility = AvroBio_coa_config_settings[6]
...
coa.merge_range('I19:L20',' Assay Issue Date', bold_border_format)
...
coa.merge_range('M19:Q20','', normal_border_format)
...
for data_file in file_path_list:
    ...
    coa.merge_range('L' + str((num * 2) + 38) + ':O' + str((num * 2) + 39), str(round(data['summary']['transduction_rate'])) + str('%'), normal_border_format)
    ...
    if lims_db_cursor.rowcount == 1:
        ...
        coa.merge_range('C' + str((num * 2) + 38) + ':K' + str((num * 2) + 39), sample_name, normal_border_format) #transduction table
        
        coa.merge_range('A' + str(num * 2 + 24) + ':B' + str(num * 2 + 25), '', normal_border_format) #identity (filled in by user)
...
##        coa.merge_range('E' + str(num * 2 + 24) + ':G' + str(num * 2 + 25), sample_name, normal_border_format)    #sample name (removed)
...
coa.merge_range('E7:H11',sponsor_info[1] + ' ' + sponsor_info[2],normal_border_format)
...
if sponsor_info[3] == None:
    ...
    # Find Project ID
lims_db_cursor.execute("SELECT idprojects FROM samples WHERE idorders = '" + str(order_id) + "' AND cnumber = '" + str(cnum) + "'")
projects_query = lims_db_cursor.fetchone()

if projects_query == None:
    print('Project ID Not Found')
else:
    coa.merge_range('E17:H20',projects_query[0],normal_border_format)
...
if q_statement == True:
    ...
    coa.merge_range('A' + str(num_files * 4 + 40) + ':Q' + str(num_files * 4 + 54), quality_statement, normal_text_wrapped_format)
