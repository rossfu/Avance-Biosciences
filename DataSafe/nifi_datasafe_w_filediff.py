...
# Database
lims_db = mysql.connector.connect(host='192.168.21.239', user=os.environ['LIMS_USER'], password=os.environ['LIMS_PASS'], database='avancelims', auth_plugin='mysql_native_password')
lims_db_cursor = lims_db.cursor()


# Log
call_str = '\n\n' + str(datetime.now()) + f"path: {path}\nfilename: {filename}\ninstrument: {instrument}\nasset: {asset}\nhome_path: {home_path}\n\n"
print(call_str)
...
if project_folder_exist or idproj == '': #copy as is IF there is no projid OR if there is a projectID folder (which is what the archive wants to have)
    workfolder_root_path = rf"\\softtest\quality\hometree\DataWorkFolder\{year}\{instrument}\{asset}"
    archive_root_path = rf"\\softtest\quality\FDABackup\{year}\{instrument}\{asset}"
    ...
    workfolder_root_path = rf"\\softtest\quality\hometree\DataWorkFolder\{year}\{instrument}\{asset}\{idproj}"
    archive_root_path = rf"\\softtest\quality\FDABackup\{year}\{instrument}\{asset}\{idproj}"
    ...
    if os.path.exists(dest_path) == True:
        ...
        def compare_text_files(file1_path, file2_path):
            ...
            # If encoding error occurs, try opening files with latin-1 encoding
                ...
                with open(file1_path, 'r', encoding='latin-1', errors="ignore") as file1:
                    file1_lines = file1.readlines()
                with open(file2_path, 'r', encoding='latin-1', errors="ignore") as file2:
                    file2_lines = file2.readlines()
...
def copy_and_hash_check(source, dest, orig_hash, ctime, mtime, username, instrument, asset, proj, summary):
    ...
    try:
        ...
        try:
            ...
            with open('C:\\nifi-1.24.0\\log.txt', 'a') as file: file.write(f'2nd copy err - datasafe! {dest}\n')
    print('copied')
