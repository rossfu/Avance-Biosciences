...
if execute_style == 'Python':
    ...
    Project_Output_Path = r'C:\Users\efu\Desktop\TestData\Project_Results'
    Excel_Template_Path = r'C:\Users\efu\Documents\JDFTemplate_EmptyForm10_Macro.xlsm'
    SlopeMin = -3.7
    SlopeMax = -3.1
    LOD = 2.5
    LOQ = 25
...
if execute_style == 'VBA':
    ...
    if '"' in Data_Directory_Path:
        Data_Directory_Path = Data_Directory_Path.replace('"', '')
        
    if '"' in Project_Output_Path:
        Project_Output_Path = Project_Output_Path.replace('"', '')
        
    if '"' in Excel_Template_Path:
        Excel_Template_Path = Excel_Template_Path.replace('"', '')
    ...
    for x in range(1,len(form10)):  
        if (form10['Sample Name'][x]) == (form10['Sample Name'][x-1]):
            ...
            #Large diff and above LOQ
                ...
                Ct_diff_list.append('PROBLEM: LARGE DIFFERENCE ' + str(float(form10['Ct'][x]) - float(form10['Ct'][x-1])) + ' (Qty Mean > LOQ)')                             
            else:
    ...
    for x in range(0,len(form10)): 
        if form10['Sample Name'][x].lower().endswith('spk'):
            ...
            if SPK_avg == 'Undetermined':
                ...
                Inhibition_true_false.append('True')
                SPK_value_differences.append('2.0')
            else:
...
def execute(Project_ID, Data_Directory_Path, Excel_File_Name, Project_Output_Path, Excel_Template_Path, SlopeMin, SlopeMax, LOQ, LOD, R2):
...
#Error Logs
    ...
    #Create Data_Directory_Path Log
    if os.path.exists(Data_Directory_Path + '\\Audit_Logs') == False: #Audit Logs
        os.mkdir(Data_Directory_Path + '\\Audit_Logs')


    #Audit Log copied at end of run
...
#System Validation
    ...
    if os.path.exists(Data_Directory_Path) == False:
        print('Data_Directory_Path does not exist, cant run program')
        run_log('Data_Directory_Path does not exist, cant run program')
        print('')
        input("Press Enter to close the program window")
        os._exit(0)

    if os.path.exists(Excel_Template_Path) == False:
        print('Excel_Template_Path does not exist, cant run program')
        run_log('Excel_Template_Path does not exist, cant run program')
        print('')
        input("Press Enter to close the program window")
        os._exit(0)
    ...
    if pd.__version__ != '1.3.1':
        print('This program is only validated on Pandas ver 1.3.1, You are using ' + str(pandas.__version__))
        run_log('This program is only validated on Pandas ver 1.3.1, You are using ' + str(pandas.__version__))
        print('')
        input("Press Enter to close the program window")
        os._exit(0)
...
#Find and Sort .TXT Raw Data Files
    ...
    #if os.path.exists(Project_Output_Path) == False:               #Already created above
    #    os.mkdir(Project_Output_Path)
...
#Set Up Informational .txt Files
    ...
    if Plate_Info_Exists == False: 
        with open(Project_Output_Path + '\\' + 'Plate_Information.txt', 'w', newline='') as Plate_Information:
            Plate_Information.write('Plate\tDate_Processed\tFile_Length\tPlate_Failed\tSamples_Failed\tAC_qty1\tAC_qty2\tRun_Success')
    ...
    #Dealing with unexpected program run termination ex. Computer shutdown
...
#Write Raw Data to Excel
                ...
                print('7900 file: ' + data_file + ' is written')
                run_log('7900 file: ' + data_file + ' is written\n\n')
...
#More Cleaning: Drop the bottom (floats), set the slope, R2, Drop nan samples
                ...
                form10['R^2'] = float(form10.iloc[-6,2][:-1])
                ...
                for i in range(len(form10)):
                    ...
                    if form10['Sample Name'][i].lower() == 'assay control':
                        form10.iloc[i, 2] = 'AC'
...
#Process Quant File
                    ...
                    #    print(str(form10_unsorted['Sample Name'][i]) + ' is type: ' + str(type(form10_unsorted['Sample Name'][i])))
                    ...
                    for i in range(len(form10_unsorted)):
                        ...
                        if form10_unsorted['Sample Name'][i].lower() == 'assay control':
...
##########################################################################################################################################################################
        ...
        if is_7500 == True: #Hello 7500
            ...
            form10['YIntercept'] = pd.Series(form10_slope_stuff['Intercept'][0])
...
#Form 10 Table
        ...
        #for x in range(len(form10['Sample Name'])):
        #    print(form10['Sample Name'][x])
...
#ReRun Status (Plates)
        ...
        for x in range(0,len(Replicate_Differences['Sample Name'])):
            if Replicate_Differences['Sample Name'][x].startswith('STD'):
                ...
                if str(Replicate_Differences['Ct diff'][x]).startswith('PROBLEM') or '.' in str(Replicate_Differences['Ct diff'][x]) and abs(Replicate_Differences['Ct diff'][x]) > 1: #Find Problems
...
#Re_Run_Samples Report
        ...
        for Replicate_value_index in range(0,len(Replicate_Differences)):
            ...
            if str(Replicate_Differences['Ct diff'][Replicate_value_index]).startswith('PROBLEM'):
                ...
                if Replicate_Differences['Sample Name'][Replicate_value_index].startswith('C'): #C name req for replicate diff
...
#######
        ...
        if len(nan_indexes) > 0: #Nan samples statement
            plate_report_ws['B' + str(Paste_row+33)] = 'NaN samples'
            plate_report_ws['C' + str(Paste_row+33)] = 'at idx ' + str(nan_indexes)
            plate_report_ws['D' + str(Paste_row+33)] = 'were ignored in calculations'
...
#AC Report
        ...
        if AC_report_df['AC_Value_Exists'][0] == True:
            ...
            with open(Project_Output_Path + '\\' + 'Plate_Information.txt', 'a', newline = '\r') as Plate_Information:
                Plate_Information.write('\n'+data_file.split('\\')[-1][:-4]+'\t'+time.asctime()+'\t'+str(len(form10)+12)+'\t'+str(Plate_Fail)+'\t'+str(Samples_Failed)+'\t\t\tFalse') #No AC Values
            Plate_Information.close()
        ...
        if len(AC_qty_list) > 0:
            ...
            ac_report_ws['I2'] = statistics.stdev(AC_qty_list) / ((sum(AC_qty_list))/len(AC_qty_list))
        ...
        ac_report_ws['M2'] = len(Plate_Info_DF['Plate_Failed'])
...
if execute_style == 'Python':
    ...
    try:
        execute(Project_ID, Data_Directory_Path, Excel_File_Name, Project_Output_Path, Excel_Template_Path, SlopeMin, SlopeMax, LOQ, LOD, R2)
...
#CMD Line: python C:\AutoForm10.py "1" "2" "3" "4" "5" "6" "7" "8" "9"
    ...
    if __name__ == '__main__': #when command line calls our python file, do:
        ...
        try:
            execute(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10])
