...
#Developed by Ross Fu 07/27/21 - 11/20/21 - 2/25/22
...
################################################################ 
#Differences between this and Master Auto Form 10*


#Healthy samples printed in F10
#F10 data pasted to column D
#7500 slope info is gathered more robustly (nrow=1)
#Does not write to F11
#Bad files get excluded and extra ending statement
#Deal with having SPK instead of SPK control (& NaN sample name)
#Allows .xlsx data files
#no column width adjusting in form10
#
#close python window upon success
#open results file after run on F10*
#
#Delete Assign Filename/Sharepath
#Hard code filename (ProjectID_yyyymmddhhmm.xlsx)*
#Hard code sharepath (qpcr Share\ProjectID\ProjectID_QACAR_biodistribution*
#hard code final sharepath (data\QACAR)*
#copy transfer from sharepath to final sharepath*
#hash check copied files*
################################################################
...
if execute_style == 'Python': #Python execution cannot exclude plates yet
    ...
    Excel_File_Name = 'Updated_F10_Results.xlsm'
    Final_Share_Path = r'C:\Users\efu\Desktop\F10Test'
    Excel_Template_Path = r'C:\Users\efu\Desktop\F10Test\Updated_F10_Test.xlsm'
    SlopeMin = -3.7
    SlopeMax = -3.1
    LOD = 2.5
    LOQ = 25
    ...
    SCS_Path_that_ran_program = r'C:\Users\efu\Desktop\F10Test\Updated_F10_Test.xlsm'
...
now = datetime.now()
ymdhm = Final_Share_Path.split('\\')[-1]

Temp_Share_Path = r'\\birch\operations\QACAR' + '\\' + Project_ID + '\\' + ymdhm
Final_Share_Path_root = '\\'.join(Final_Share_Path.split('\\')[:-2])
    ...
    for x in range(1,len(form10)):
        if (form10['Sample Name'][x]) == (form10['Sample Name'][x-1]) and str(form10['Sample Name'][x].lower()).startswith('spk') == False and str(form10['Sample Name'][x]).startswith('N') == False:
            if math.isnan(form10['Quantity'][x]) and math.isnan(form10['Quantity'][x-1]) == False:
                ...
                QtyMean_List.append((float(form10['Quantity'][x-1]))/2) #1 nan qty still gotta divide by 2 for qty mean
            elif math.isnan(form10['Quantity'][x-1]) and math.isnan(form10['Quantity'][x]) == False:
                Sample_names_list.append(form10['Sample Name'][x])
                QtyMean_List.append(float(form10['Quantity'][x])/2) #1 nan qty still gotta divide by 2 for qty mean
            else:
                Sample_names_list.append(form10['Sample Name'][x])
...
def run_log(message):
    with open(Temp_Share_Path + '\\' + Project_ID + '_' + Runtime + '_AF10_run_log.txt', 'a') as z:
        z.write(message)
...
def execute(Project_ID, Data_Directory_Path, Excel_File_Name, Final_Share_Path, Excel_Template_Path, SlopeMin, SlopeMax, LOQ, LOD, R2, Inhib_Diff_Criteria, Repl_Diff_Criteria, Repl_Diff_Criteria_for_STD_1_to_7, Repl_Diff_Criteria_for_STD_8, SCS_Path_that_ran_program):
...
#Run Logs
    ...
    #Create Final Share Path Dirs
    if os.path.exists(Final_Share_Path_root + '\\' + Project_ID) == False:   
        os.mkdir(Final_Share_Path_root + '\\' + Project_ID)

    if os.path.exists(Final_Share_Path_root + '\\' + Project_ID + '\\' + ymdhm) == False: 
        os.mkdir(Final_Share_Path_root + '\\' + Project_ID + '\\' + ymdhm)
        
    #Create Temp_Share_Path Log
    if os.path.exists(Temp_Share_Path) == False: #Temp_Share_Path
        os.mkdir(Temp_Share_Path)

    with open(Temp_Share_Path + '\\' + Project_ID + '_' + Runtime + '_AF10_run_log.txt', 'w') as z:
        z.write('User: ' + os.environ['USERNAME'] + '\n')
        z.write('Computer Name: ' + os.environ['COMPUTERNAME'] + '\n\n')


    #Dispose of the VBS Script
    if os.path.exists(Temp_Share_Path + '\\' + 'VBS_for_execute_AutoForm10.vbs'):
        os.remove(Temp_Share_Path + '\\' + 'VBS_for_execute_AutoForm10.vbs')
...
#Find and Sort .TXT Raw Data Files
    ...
    Need_modified_7900_folder = True
    ...
    for root, dirs, files in os.walk(Temp_Share_Path):
        ...
        if 'Modified_7900_data_files' in dirs:
            Need_modified_7900_folder = False
    ...
    if Need_modified_quant_folder == True:
        os.mkdir(Temp_Share_Path + '\\Modified_Quant_data_files')

    if Need_modified_7500_folder == True:
        os.mkdir(Temp_Share_Path + '\\Modified_7500_data_files')

    if Need_modified_7900_folder == True:
        os.mkdir(Temp_Share_Path + '\\Modified_7900_data_files')
...
#Set Up Excel
    ...
    workbook_path = Temp_Share_Path + '\\' + Excel_File_Name
...
##    form11_ws = wb['Form11_Analysis']
    ...
    if os.path.exists(Temp_Share_Path + '\\' + Excel_File_Name):    #VBA made the resulting workbook, excluded plate info is transferred
        exclude_plates = pd.read_excel(Temp_Share_Path + '\\' + Excel_File_Name, sheet_name = 'QACAR Controls', header=0, usecols = ['Exclude these plates', 'Reason'])
        ...
        exclude_plates = pd.read_excel(SCS_Path_that_ran_program, sheet_name = 'QACAR Controls', header=0, usecols = ['Exclude these plates', 'Reason'])
...
#Set Up Informational .txt Files
    ...
    if Plate_Info_Exists == False: 
        with open(Temp_Share_Path + '\\' + 'Plate_Information.txt', 'w', newline='') as Plate_Information:
        ...
        Plate_Info_DF = pd.read_csv(Temp_Share_Path + '\\' + 'Plate_Information.txt', sep = '\t', lineterminator = '\r', header = 0)

    else:
        Plate_Info_DF = pd.read_csv(Temp_Share_Path + '\\' + 'Plate_Information.txt', sep = '\t', header = 0)
    ...
    if Re_Run_Sample_Info_Exists == False:
        with open(Temp_Share_Path + '\\' + 'Re_Run_Sample_Info.txt', 'w') as Re_Run_Sample_Info:
        ...
        Re_Run_Sample_Info_Dataframe = pd.read_csv(Temp_Share_Path + '\\' + 'Re_Run_Sample_Info.txt', sep = '\t', lineterminator = '\n', header = 0)

    else:
        Re_Run_Sample_Info_Dataframe = pd.read_csv(Temp_Share_Path + '\\' + 'Re_Run_Sample_Info.txt', sep = '\t', header = 0)
...
#Early removal of failed plates from Plate_Info and Re_Run_Samples files to deal with unexpected program termination ex. Computer shutdown
    ...
    Plate_Info_DF.to_csv(Temp_Share_Path + '\\' + 'Plate_Information.txt', sep='\t', index = False, header = True, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
...
##    #Determine Form11 empty row
##    F11 = pd.read_excel(SCS_Path_that_ran_program, sheet_name = 'Form11_Analysis', header=0)
##
##    for i in range(2401):
##        if F11['Sample ID or Plate Map Label'][i] == 'Empty Well':
##            Form11_empty_row = i+2
##            break
##
            
    Form11_transfer_table_row = 1
    Form11_transfer_table_Paste_Row = 20
...
#Classify the Data file
        ...
        if is_7500 == False:
            ...
            if test_the_file.columns[0] == 'SDS 2.4': #Hello 7900
                ...
                Header = pd.DataFrame(columns=['SDS 2.4', 'AQ Results'])
                Header = Header.append({'SDS 2.4':'Filename', 'AQ Results':data_file.split('\\')[-1]}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'PlateID'}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'Assay Type', 'AQ Results':'Absolute Quantification'}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'Run Date Time', 'AQ Results':datetime.fromtimestamp(os.path.getmtime(data_file))}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'Operator'}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'ThermalCycleParams','AQ Results':'7900'}, ignore_index=True)
                Header = Header.append({'SDS 2.4':'SampleInformation'}, ignore_index=True)                                      
                                      

                    
#Clean Raw Data
            
                form10.drop(['\r','FOS','HMD','LME','EW','BPR','NAW','HNS','HRN','EAF','BAF','TAF','CAF', 'Reporter', 'Tm Value', 'Tm Type', 'Qty StdDev', 'Ct Median', 'Ct Mean', 'Ct StdDev', 'Ct Type', 'Template Name', 'Baseline Type', 'Baseline Start', 'Baseline Stop', 'Threshold Type', 'Threshold'], axis = 1, inplace = True)



#Create Modified 7900 File
                ...
                for x in range(len(form10['Well'])):
                    if form10['Well'][x] == 'Slope':
                        form10.drop(form10.tail(len(form10)-x).index, inplace = True)
                        break
                        
                for i in range(len(form10)):
                    if type(form10['Sample Name'][i]) != float:
                        if form10['Sample Name'][i] != form10['Sample Name'][i].lstrip():
                            form10.iloc[i, 1] = form10['Sample Name'][i].lstrip()

                form10 = form10.sort_values(by=['Sample Name'], ignore_index = True)


                #Modify column order for DUMMY
                fill_1 = []
                fill_2 = []
                form10[' '] = pd.Series(fill_1)
                form10['  '] = pd.Series(fill_2)
                cols = form10.columns.tolist()
                ...
                for i in range(len(form10)):
                    ...
                    if pd.isna(form10['Sample Name'][i]) == True:
                        form10.iloc[i, 1] = 'N/A'
                
                    if form10['Sample Name'][i].upper() == 'SPK':
                        form10.iloc[i, 1] = 'SPK CONTROL'
                    ...
                    if form10['Sample Name'][i] != form10['Sample Name'][i].upper():
                        form10.iloc[i, 1] = form10.iloc[i, 1].upper()
                    ...
                    if form10['Sample Name'][i] != form10['Sample Name'][i].replace(' ', '') and form10['Sample Name'][i].startswith('SP') == False:
                        form10.iloc[i, 1] = form10['Sample Name'][i].replace(' ', '')

                    if form10['Sample Name'][i] != form10['Sample Name'][i].lstrip():
                        form10.iloc[i, 1] = form10['Sample Name'][i].lstrip()

                    if form10['Sample Name'][i].startswith('SPK') == False:
                        form10.iloc[i, 1] = str(form10['Sample Name'][i]).strip()
                ...
                Header.to_csv(Temp_Share_Path + '\\Modified_7900_data_files\\MODIFIED_7900_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='w',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
                form10.to_csv(Temp_Share_Path + '\\Modified_7900_data_files\\MODIFIED_7900_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='a',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')


                    
#Write Raw Data to Excel

                open_data_file = open(Temp_Share_Path + '\\Modified_7900_data_files\\MODIFIED_7900_FILE_' + data_file.split('\\')[-1], 'r')
...
##                        form10.drop(form10.tail(len(form10)-x).index, inplace = True)
                ...
                for x in range(0,len(form10)):
                    if pd.isna(form10['Sample Name'][x]) == True or form10['Sample Name'][x] == 'N/A':
...
##
##                    if form10['Sample Name'][i] != form10['Sample Name'][i].lstrip():
##                        form10.iloc[i, 2] = form10['Sample Name'][i].lstrip()
##
...
#Process Quant File
                    ...
                    for i in range(len(form10_unsorted)):
                        ...
                        if pd.isna(form10_unsorted['Sample Name'][i]) == True:
                            form10_unsorted.iloc[i, 1] = 'N/A'
                        
                        if form10_unsorted['Sample Name'][i].upper() == 'SPK':
                    ...
                    #Reorder columns
                    form10 = form10[['Well', 'Sample Name', 'Target Name', 'Task', 'Ct', 'Quantity', 'Qty Mean', 'Slope', 'R^2', 'Y_Intercept']]
                        

                    #Add a Header/Footer
                    Header = pd.DataFrame(columns=['Quant Studio Results',' '])
##                    Header = Header.append(['Quant Studio Results',' ','For File:',data_file.split('\\')[-1][:-4],' ',' ','Sample Information',' '])

                    Header = Header.append({'Quant Studio Results':''}, ignore_index=True)
                    Header = Header.append({'Quant Studio Results':'For File:', ' ':data_file.split('\\')[-1]}, ignore_index=True)
                    Header = Header.append({'Quant Studio Results':''}, ignore_index=True)
                    Header = Header.append({'Quant Studio Results':'Run Date', ' ':datetime.fromtimestamp(os.path.getmtime(data_file))}, ignore_index=True)
                    Header = Header.append({'Quant Studio Results':''}, ignore_index=True)
                    Header = Header.append({'Quant Studio Results':'SampleInformation'}, ignore_index=True)     
                    Header = Header.append({'Quant Studio Results':''}, ignore_index=True)
...
#Create Modified Quant Studio File
                    ...
                    Header.to_csv(Temp_Share_Path + '\\Modified_Quant_data_files\\MODIFIED_QUANTSTUDIO_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='w',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
                    form10.to_csv(Temp_Share_Path + '\\Modified_Quant_data_files\\MODIFIED_QUANTSTUDIO_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='a',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
                    Footer.to_csv(Temp_Share_Path + '\\Modified_Quant_data_files\\MODIFIED_QUANTSTUDIO_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='a',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
                    

#Write QS Data to Excel

                    open_data_file = open(Temp_Share_Path + '\\Modified_Quant_data_files\\MODIFIED_QUANTSTUDIO_FILE_' + data_file.split('\\')[-1], 'r')
                ...
                print('')
                print('The file: ' + data_file.split('\\')[-1] + ' does not match any valid formats')
                print('The analysis must exclude: ' + data_file.split('\\')[-1])
                run_log(time.asctime() + '\n')
                run_log('The file: ' + data_file.split('\\')[-1] + ' does not match any valid formats\n')
                run_log('The analysis must exclude: ' + data_file.split('\\')[-1] + '\n')

                #Mark as excluded in txt files
                Plate_Info_DF = Plate_Info_DF.append({'Plate': data_file.split('\\')[-1], 'Date_Processed': 'Excluded because: Program determined bad file format', 'Run_Success':True}, ignore_index=True)
                Re_Run_Sample_Info_Dataframe = Re_Run_Sample_Info_Dataframe.append({'Need_Re_Run_Plate':data_file.split('\\')[-1],'Need_Re_Run_Samples':'All','Reason_Re_Run': 'Excluded because: Program determined bad file format', 'Run_Success':True}, ignore_index=True)


                bad_files.append(data_file.split('\\')[-1])
...
##########################################################################################################################################################################
        ...
        if is_7500 == True: #Hello 7500
            ...
            if data_file.endswith('.xlsx'):
                ...
                #infile, outfile, csvintermediary
##                with open(data_file, 'r', encoding='utf-8', errors='ignore') as infile, open(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_FILE_' + data_file.split('\\')[-1], 'w') as outfile, open(Temp_Share_Path + '\\Modified_7500_data_files\\CSV_Version_of_Xlsx_' + (data_file.split('\\')[-1]).split('.')[0] + '.csv', 'w', encoding='utf-8') as csv_intermediary:
##
##                    contents = infile.readlines()
##                    for line in contents:
##                        csv_intermediary.write(line)   #xlsx to csv                                                                                                                                                                                                             
##                                                                                                                                                                                                                                    
##                    #try csv writer on the csv intermediary
##                        
##                    inputs = csv.reader(csv_intermediary)
...
##                infile.close()
##                outfile.close()
##                csv_intermediary.close()




                xlsx_data = pd.read_excel(data_file, (data_file.split('\\')[-1]).split('.')[0], dtype=str)
                xlsx_data.to_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_xlsx_FILE_' + (data_file.split('\\')[-1]).split('.')[0] + '.csv', encoding = 'utf-8', index=False)

                find_form10_data = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_xlsx_FILE_' + (data_file.split('\\')[-1]).split('.')[0] + '.csv', usecols = [0])



            else: #.csv
                with open(data_file, 'r', encoding='utf-8', errors='ignore') as infile, open(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_FILE_' + data_file.split('\\')[-1], 'w') as outfile:
                ...
                find_form10_data = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_FILE_' + data_file.split('\\')[-1], usecols = [0])
            ...
            if data_file.endswith('.csv'):
                form10_slope_stuff = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_FILE_' + data_file.split('\\')[-1], header = Index_where_slope_r_y_is, nrows = 1)    #, usecols = ['Slope', 'Intercept', 'R2'])
                form10_unsorted = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_FILE_' + data_file.split('\\')[-1], header = Index_where_data_begins, usecols = ['Well', 'Sample Name', 'Detector', 'Task','Ct','Qty','Mean Qty'])
            else:
                form10_slope_stuff = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_xlsx_FILE_' + (data_file.split('\\')[-1]).split('.')[0] + '.csv', header = Index_where_slope_r_y_is, nrows = 1)    #, usecols = ['Slope', 'Intercept', 'R2'])
                form10_unsorted = pd.read_csv(Temp_Share_Path + '\\Modified_7500_data_files\\RE_ENCODED_7500_xlsx_FILE_' + (data_file.split('\\')[-1]).split('.')[0] + '.csv', header = Index_where_data_begins, usecols = ['Well', 'Sample Name', 'Detector', 'Task','Ct','Qty','Mean Qty'])
...
##            Header = Header.append(['7500 Results',' ','For File:',data_file.split('\\')[-1][:-4],' ',' ','Sample Information',' '])
            ...
            Header = pd.DataFrame(columns=['7500 Results',' '])
            Header = Header.append({'7500 Results':''}, ignore_index=True)
            Header = Header.append({'7500 Results':'For File:', ' ':data_file.split('\\')[-1]}, ignore_index=True)
            Header = Header.append({'7500 Results':''}, ignore_index=True)
            Header = Header.append({'7500 Results':'Run Date', ' ':datetime.fromtimestamp(os.path.getmtime(data_file))}, ignore_index=True)
            Header = Header.append({'7500 Results':''}, ignore_index=True)
            Header = Header.append({'7500 Results':'SampleInformation'}, ignore_index=True)     
            Header = Header.append({'7500 Results':''}, ignore_index=True)
...
#Create Modified 7500 File
            ...
            Header.to_csv(Temp_Share_Path + '\\Modified_7500_data_files\\MODIFIED_7500_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='w',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
            form10.to_csv(Temp_Share_Path + '\\Modified_7500_data_files\\MODIFIED_7500_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='a',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
            Footer.to_csv(Temp_Share_Path + '\\Modified_7500_data_files\\MODIFIED_7500_FILE_' + data_file.split('\\')[-1], sep = '\t', mode='a',index=False, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
            

#Write 7500 Data to Excel

            open_data_file = open(Temp_Share_Path + '\\Modified_7500_data_files\\MODIFIED_7500_FILE_' + data_file.split('\\')[-1], 'r')
...
#Form 10 Table
        ...
        if AC_exist:
            ws['AF' + str(Paste_row)] = 'Form11 Samples'
            ws['T' + str(Paste_row)] = 'Healthy Samples'
            ws['U' + str(Paste_row)] = 'Qty Mean'
        else:
            ws['AF' + str(Paste_row-1)] = 'Form11 Samples'
            ws['T' + str(Paste_row-1)] = 'Healthy Samples'
            ws['U' + str(Paste_row-1)] = 'Qty Mean'
...
#ReRun Status (Plates)
        ...
        #Plate Fail Reason in F10
        Paste_row = row_where_file_ends + 11
        ws['T' + str(Paste_row)] = 'Plate Fail Reason'


        if Plate_Fail:
            ws['T' + str(Paste_row+1)] = ', '.join(plate_fail_reason)
        else:
            ws['T' + str(Paste_row+1)] = 'None'
...
#Re_Run_Samples Report
        ...
        #Collect Healthy Samples Phase 1 of 2
        plate_potential_healthy_samples = []
        
        if Plate_Fail == False:
            plate_potential_healthy_samples = Sample_Name_List
        ...
        for Inhibited_Sample_Index in range(len(SPK_Ct['Inhibition?'])):
            ...
            if SPK_Ct['Inhibition?'][Inhibited_Sample_Index] == 'True':
                ...
                if SPK_Ct['Sample Name'][Inhibited_Sample_Index].startswith('N') == False and SPK_Ct['Sample Name'][Inhibited_Sample_Index].startswith('S') == False and SPK_Ct['Sample Name'][Inhibited_Sample_Index].startswith('A') == False: 
                    inhibited_samples.append(SPK_Ct['Sample Name'][Inhibited_Sample_Index].split('+')[0])
...
#######
        ...
        #Collect Healthy Samples Phase 2 of 2

        plate_healthy_sample_exist = False
        plate_healthy_samples = []
        plate_healthy_samples_qty_mean = []
        ...
        if Plate_Fail == False:
            for i in range(len(QuantityMeans)):
                if QuantityMeans['Sample Name'][i] not in replicate_diff_issue_samples and QuantityMeans['Sample Name'][i] not in inhibited_samples and QuantityMeans['Sample Name'][i] not in above_std_curve_range_list and QuantityMeans['Sample Name'][i].startswith('STD') == False and QuantityMeans['Sample Name'][i].startswith('AC') == False:
                    plate_healthy_sample_exist = True
                    plate_healthy_samples.append(QuantityMeans['Sample Name'][i])
                    plate_healthy_samples_qty_mean.append(QuantityMeans['Qty Mean'][i])
                    

        if plate_healthy_sample_exist == True:
            for i in range(len(plate_healthy_samples)):                
                ws['T' + str(Paste_row)] = plate_healthy_samples[i] #f10
                ws['U' + str(Paste_row)] = plate_healthy_samples_qty_mean[i]
                Paste_row += 1


                if plate_healthy_samples[i].startswith('C'):
                    ws['AF' + str(Form11_transfer_table_Paste_Row)] = plate_healthy_samples[i] #f11 transfer table
                    ws['AG' + str(Form11_transfer_table_Paste_Row)] = plate_healthy_samples_qty_mean[i]
                    
                    Form11_transfer_table_row += 1
                    Form11_transfer_table_Paste_Row += 1
                    

                    if Form11_transfer_table_row == 25: #next table
                        Form11_transfer_table_row = 1
                        Form11_transfer_table_Paste_Row += 85

                
                
##                form11_ws['H' + str(Form11_empty_row)] = plate_healthy_samples[i] #f11
##                form11_ws['I' + str(Form11_empty_row)] = plate_healthy_samples_qty_mean[i]
##                Form11_empty_row += 1
...
#            with open(Temp_Share_Path + '\\' + 'Plate_Information.txt', 'a', newline = '\r') as Plate_Information:
...
#Save Re Run Sample Info
        Re_Run_Sample_Info_Dataframe.to_csv(Temp_Share_Path + '\\' + 'Re_Run_Sample_Info.txt', sep='\t', index = False, header = True, quoting=csv.QUOTE_NONE, quotechar='',  escapechar='$')
...
#When the For each file loop is ended
    ...
    plt.savefig(Temp_Share_Path + '\\' + 'y_int.png')

    img = openpyxl.drawing.image.Image(Temp_Share_Path + '\\' + 'y_int.png')
    img.anchor = 'D3'
    ...
    plt.savefig(Temp_Share_Path + '\\' + 'y_trend.png')
    
    img2 = openpyxl.drawing.image.Image(Temp_Share_Path + '\\' + 'y_trend.png')
    img2.anchor = 'K2'
...
##    ws.column_dimensions['AL'].width = 17
...
##    ws.column_dimensions['AG'].width = 20
    ...
    plate_report_ws.column_dimensions['N'].width = 9
    plate_report_ws.column_dimensions['O'].width = 4
...
#Bad Files statement
    ...
    if len(bad_files) > 0:
        ...
        run_log('\n\n#######################################################################################\n\n')
        print('')
        print('Also Excluding Plates: ' + ', '.join(bad_files))
        run_log('Also Excluding Plates: ' + ', '.join(bad_files) + '\n\n')
        print('')
        for i in bad_files:
            print('Excluding ' + i + ' because: Program determined a bad file format')
            run_log('Excluding ' + i + ' because: Program determined a bad file format\n')
        print('')
        print('')
        run_log('\n')
        print('#######################################################################################')
...
#Save Back Up File
    ...
    if backup_file_exists == True:
        os.rename(Backup_Filename, Temp_Share_Path + '\\' + 'Form10_BackUp_' + Excel_File_Name[:-5] + '_' + str(now.strftime('%m-%d-%Y_time_%H_%M')) + '.xlsm')
        shutil.copyfile(Temp_Share_Path + '\\' + Excel_File_Name, Temp_Share_Path + '\\' + 'Form10_BackUp_' + Excel_File_Name[:-5] + '_' + str(now.strftime('%m-%d-%Y_time_%H_%M')) + '.xlsm')
    else:
        shutil.copyfile(Temp_Share_Path + '\\' + Excel_File_Name, Temp_Share_Path + '\\' + 'Form10_BackUp_' + Excel_File_Name[:-5] + '_' + str(now.strftime('%m-%d-%Y_time_%H_%M')) + '.xlsm')
...
#Delete Y_Int Image
    ...
    if os.path.exists(Temp_Share_Path + '\\' + 'y_int.png'):
        os.remove(Temp_Share_Path + '\\' + 'y_int.png')

    if os.path.exists(Temp_Share_Path + '\\' + 'y_trend.png'):
        os.remove(Temp_Share_Path + '\\' + 'y_trend.png')
...
if execute_style == 'Python':
    ...
    try:
        execute(Project_ID, Data_Directory_Path, Excel_File_Name, Temp_Share_Path, Excel_Template_Path, SlopeMin, SlopeMax, LOQ, LOD, R2, Inhib_Diff_Criteria, Repl_Diff_Criteria, Repl_Diff_Criteria_for_STD_1_to_7, Repl_Diff_Criteria_for_STD_8, SCS_Path_that_ran_program)
        ...
        #Audit Log
        shutil.copyfile(Temp_Share_Path + '\\' + Project_ID + '_' + Runtime + '_AF10_run_log.txt', Data_Directory_Path + '\\' + 'Audit_Logs' + '\\' + Project_ID + '_' + Runtime + '_AF10_run_log.txt')
...
#Just Kidding #Tera Transfer

if not os.path.exists(Final_Share_Path):
    os.makedirs(Final_Share_Path)

teracopy = r'C:\Program Files\TeraCopy\TeraCopy.exe'
print('starting tera transfer')
subprocess.call([teracopy, 'copy', Temp_Share_Path, Final_Share_Path_root + '\\' + Project_ID])
...
for root,dirs,files in os.walk(Final_Share_Path):
    for names in files:
        dest_files += 1

if source_files != dest_files:
    copy_problem = True
    print('tera transfer was unsuccessful')
    print('num source files: ' + source_files)
    print('num dest files: ' + dest_files)
    print('')


else: #delete temp share files/folders
    shutil.rmtree(Temp_Share_Path)


#Upload results to LIMS??
teralog = r'C:\teracopy\teracopy_log_files.ps1'
subprocess.call('powershell.exe ' + teralog)
...
if len(bad_files) > 0 or copy_problem:
    input("Press Enter to close the program window")
else:
    print('Closing python window in 30 seconds')


#Open Results File
results = Final_Share_Path + '\\' + Excel_File_Name
